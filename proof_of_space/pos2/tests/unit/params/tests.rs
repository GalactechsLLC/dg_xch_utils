...
#[test]
fn final_k32_chaining_range_ends_at_u64_max_without_overflow() {
    let params = ProofParams::new([42; 32].into(), 32, 2, false).unwrap();
    let last = u64::from(params.num_chaining_sets() - 1);
    assert_eq!(params.chaining_set_range(last).end, u64::MAX);
}
