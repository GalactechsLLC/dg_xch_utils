...
From a top-level workspace crate, depend on `dg_xch_pos2 = { path = "../proof_of_space/pos2" }`; enable `features = ["vulkan"]` for GPU compute consumers. Application startup belongs to `dgx plotter` or the desktop. These plotting primitives do not wait for node synchronization.
