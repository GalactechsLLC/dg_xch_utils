...
impl<'a> Chainer<'a> {
    pub fn search(
        &self,
        sets: [&[u64]; NUM_CHALLENGE_SETS],
        limits: SearchLimits,
        cancelled: &std::sync::atomic::AtomicBool,
    ) -> Result<Vec<Chain>, SearchError> {
        let ranges = self.core.select_challenge_sets(self.challenge).ranges;
        for (index, set) in sets.iter().enumerate() {
            if set.len() > 4096
                || set
                    .iter()
                    .any(|fragment| !ranges[index].contains(*fragment))
                || set.windows(2).any(|pair| pair[0] >= pair[1])
            {
                return Err(SearchError::InvalidSets);
            }
        }
        let round_keys = self
            .core
        ...
        let mut hashes = 0u64;
        for start in 0..NUM_CHALLENGE_SETS {
            let mut fragments = [0; NUM_CHAIN_LINKS];
            let mut states = [0; NUM_CHAIN_LINKS + 1];
            let mut positions = [0usize; NUM_CHAIN_LINKS];
            let mut depth = 0;
            loop {
                if cancelled.load(std::sync::atomic::Ordering::Relaxed) {
                    return Err(SearchError::Cancelled);
                }
                let set = sets[(start + depth) % NUM_CHALLENGE_SETS];
                if positions[depth] == set.len() {
                    positions[depth] = 0;
                    if depth == 0 {
                        break;
                    }
                ...
                if hashes == limits.max_hashes {
                    return Err(SearchError::WorkLimit);
                }
                let fragment = set[positions[depth]];
                positions[depth] += 1;
                hashes += 1;
                let hash = self
                    .core
                    .hashing
                    .chain_hash(states[depth] ^ fragment ^ round_keys[depth]);
                if !Self::passes_fast_filter(hash, depth) {
                    continue;
                }
                fragments[depth] = fragment;
                if depth == NUM_CHAIN_LINKS - 1 {
...
#[cfg(test)]
#[path = "../tests/unit/chainer/tests.rs"]
mod tests;
