...
use dg_xch_pos1::plots::plot_writer::{forward_propagate, proof_bytes, proof_xs};
use dg_xch_pos1::utils::bit_reader::BitReader;
...
fn packing_round_trips_through_the_verifiers_reader() {
    ...
    assert_eq!(
        dg_xch_pos1::verifier::uncompress_proof(&bytes, K as usize),
        xs
    );
}
...
fn every_stored_pair_satisfies_the_match_rule() {
    ...
    for table in 2..=7usize {
        ...
        for (i, e) in tables.table(table).iter().enumerate() {
            ...
            assert!(
                dg_xch_pos1::plots::fx_generator::fx_match(&yl, &yr),
...
fn replaying_a_proof_reproduces_the_stored_chain() {
    ...
    use dg_xch_pos1::plots::fx_generator::{fx_gen, get_proof_f1_and_meta};
    ...
    .enumerate()
    {
        ...
        while i < count {
            let mut out_y = 0u64;
            let mut out_meta = dg_xch_pos1::utils::bit_reader::BitReader::default();
            fx_gen(
                table,
                u32::from(K),
                fx[i],
...
fn the_batch_f1_agrees_with_the_verifiers_f1() {
    use dg_xch_pos1::f_calc::F1Calculator;
    use dg_xch_pos1::plots::fx_generator::get_proof_f1_and_meta;
...
fn f1_extraction_matches_individual_bits_across_block_boundaries() {
    use dg_xch_pos1::chacha8::{ChachaContext, chacha8_get_keystream, chacha8_keysetup};
    use dg_xch_pos1::constants::K_EXTRA_BITS;
    use dg_xch_pos1::plots::fx_generator::get_proof_f1_and_meta;
