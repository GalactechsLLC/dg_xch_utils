use crate::constants::{
    ...
    K_ENTRIES_PER_PARK, K_MAX_PLOT_SIZE, K_MIN_PLOT_SIZE, K_RVALUES, K_STUB_MINUS_BITS, ucdiv,
    ucdiv_t, ucdiv64,
};
use crate::encoding;
...
> PlotReader<F, T>
{
    ...
    pub async fn new(
        ...
        validate_plot_header(t.header(), *t.plot_size())?;
    ...
    pub async fn read_c3park(&self, park_index: u64) -> Result<Vec<u64>, Error> {
        ...
        let (count, deltas) = {
            ...
            let payload_size = usize::try_from(c3park_size)
                .ok()
                .and_then(|size| size.checked_sub(size_of::<u16>()))
                .ok_or_else(|| Error::new(ErrorKind::InvalidData, "Invalid C3 park size"))?;
            if usize::from(compressed_size) > payload_size {
                return Err(Error::new(
                    ErrorKind::InvalidInput,
                    format!("Invalid size for c3 deltas: {compressed_size}"),
                ));
            }
            let mut park_buffer = vec![0; payload_size];
        ...
        validate_c3_delta_count(count, deltas.len())?;
    ...
    async fn load_c2entries(&mut self) -> Result<(), Error> {
        let c2size = usize::try_from(self.file.table_size(PlotTable::C2))
            .map_err(|_| Error::new(ErrorKind::InvalidData, "C2 table exceeds address space"))?;
        if c2size != 0 {
            let k = self.file.k() as usize;
            ...
            if c2max_entries > 0 {
                ...
                buffer.try_reserve_exact(c2size).map_err(Error::other)?;
                buffer.resize(c2size, 0);
                file_lock.read_exact(&mut buffer).await?;
                self.c2_entries = Vec::new();
                self.c2_entries
                    .try_reserve_exact(c2max_entries)
                    .map_err(Error::other)?;
...
fn validate_c3_delta_count(count: usize, decoded_length: usize) -> Result<(), Error> {
    if count == 0 || count > K_CHECKPOINT1INTERVAL as usize || count > decoded_length {
        return Err(Error::new(ErrorKind::InvalidData, "Invalid C3 delta count"));
    }
    Ok(())
}
...
pub fn read_plot_header(file: &mut std::fs::File) -> Result<PlotHeader, Error> {
    ...
    validate_plot_header(&header, file.metadata()?.len())?;
    Ok(header)
}

fn validate_plot_header(header: &PlotHeader, file_size: u64) -> Result<(), Error> {
    if !(K_MIN_PLOT_SIZE..=K_MAX_PLOT_SIZE).contains(&u32::from(header.k())) {
        return Err(Error::new(
            ErrorKind::InvalidData,
            "Unsupported plot k size",
        ));
    }
    let (pointers, declared_sizes, header_size, dropped_tables) = match header {
        PlotHeader::V1(header) => (
            &header.table_begin_pointers,
            None,
            136 + u64::from(header.format_desc_len) + u64::from(header.memo_len),
            0,
        ),
        PlotHeader::V2(header) => {
            if header.version != 2
                || header.plot_flags & !1 != 0
                || header.compression_level > 9
                || (header.compression_level > 0 && header.k != 32)
                || (header.plot_flags & 1 == 0 && header.compression_level != 0)
            {
                return Err(Error::new(
                    ErrorKind::InvalidData,
            ...
            (
                &header.table_begin_pointers,
                Some(&header.table_sizes),
                207 + u64::from(header.memo_len) + u64::from(header.plot_flags & 1),
                if header.compression_level >= 9 {
                    2
                } else {
                    usize::from(header.compression_level > 0)
                },
            )
        }
        PlotHeader::GHv2_5(_) => {
    ...
    let mut sizes = [0u64; 10];
    for (index, address) in pointers.iter().copied().enumerate() {
        if index < dropped_tables && address == 0 {
            if declared_sizes.is_some_and(|sizes| sizes[index] != 0) {
                return Err(Error::new(
                    ErrorKind::InvalidData,
                    "Dropped table has a size",
                ));
            }
            continue;
        }
        if address < header_size || address >= file_size {
            return Err(Error::new(
                ErrorKind::InvalidData,
                "Plot table is outside the file",
            ));
        }
        let size = if let Some(declared_sizes) = declared_sizes {
            ...
            pointers
                ...
                .unwrap_or(file_size)
                .checked_sub(address)
                .ok_or_else(|| Error::new(ErrorKind::InvalidData, "Invalid plot table range"))?
        };
        let end = address
            .checked_add(size)
            .filter(|end| *end <= file_size && *end > address)
            .ok_or_else(|| Error::new(ErrorKind::InvalidData, "Invalid plot table range"))?;
        sizes[index] = size;
    ...
    if ranges.windows(2).any(|pair| pair[0].1 > pair[1].0) {
        return Err(Error::new(ErrorKind::InvalidData, "Plot tables overlap"));
    }
    let checkpoint_size = u64::from(header.k()).div_ceil(8);
    let checkpoint_interval = u64::from(K_CHECKPOINT1INTERVAL) * u64::from(K_CHECKPOINT2INTERVAL);
    let maximum_entries = (1u64 << (u32::from(header.k()) + 1)).div_ceil(checkpoint_interval) + 2;
    if sizes[PlotTable::C1 as usize] < checkpoint_size
        || sizes[PlotTable::C2 as usize] < checkpoint_size
        || sizes[PlotTable::C2 as usize] > maximum_entries * checkpoint_size + 1024 * 1024
    {
        return Err(Error::new(
            ErrorKind::InvalidData,
            "Invalid checkpoint table size",
        ));
    }
    Ok(())
}

fn header_bytes<'buffer>(
    remaining: &mut &'buffer [u8],
    count: usize,
) -> Result<&'buffer [u8], Error> {
    let (field, rest) = remaining
        .split_at_checked(count)
        .ok_or_else(|| Error::new(ErrorKind::InvalidData, "Truncated plot header"))?;
    *remaining = rest;
    Ok(field)
}

fn header_array<const LENGTH: usize>(remaining: &mut &[u8]) -> Result<[u8; LENGTH], Error> {
    header_bytes(remaining, LENGTH)?
        .try_into()
        .map_err(|_| Error::new(ErrorKind::InvalidData, "Invalid plot header field"))
}

fn parse_v1(mut remaining: &[u8]) -> Result<PlotHeaderV1, Error> {
    let mut plot_header = PlotHeaderV1 {
        magic: header_array(&mut remaining)?,
        id: Bytes32::parse(header_bytes(&mut remaining, 32)?)?,
        k: u8::from_be_bytes(header_array(&mut remaining)?),
        format_desc_len: u16::from_be_bytes(header_array(&mut remaining)?),
        ..Default::default()
    };
    ...
    plot_header.memo = PlotMemo::try_from(header_bytes(
        &mut remaining,
        usize::from(plot_header.memo_len),
    )?)?;
    for pointer in &mut plot_header.table_begin_pointers {
        *pointer = u64::from_be_bytes(header_array(&mut remaining)?);
    }
    Ok(plot_header)
}

fn parse_v2(mut remaining: &[u8]) -> Result<PlotHeaderV2, Error> {
    let mut plot_header = PlotHeaderV2 {
        magic: header_array(&mut remaining)?,
        version: u32::from_le_bytes(header_array(&mut remaining)?),
        id: Bytes32::parse(header_bytes(&mut remaining, 32)?)?,
        k: u8::from_be_bytes(header_array(&mut remaining)?),
        memo_len: u16::from_be_bytes(header_array(&mut remaining)?),
        ..Default::default()
    };
    plot_header.memo = PlotMemo::try_from(header_bytes(
        &mut remaining,
    ...
    plot_header.plot_flags = u32::from_le_bytes(header_array(&mut remaining)?);
    if plot_header.plot_flags & 1 == 1 {
        plot_header.compression_level = u8::from_be_bytes(header_array(&mut remaining)?);
    }
    for pointer in &mut plot_header.table_begin_pointers {
        *pointer = u64::from_be_bytes(header_array(&mut remaining)?);
    }
    for pointer in &mut plot_header.table_sizes {
        *pointer = u64::from_le_bytes(header_array(&mut remaining)?);
    }
    Ok(plot_header)
}
...
pub async fn read_plot_header_async(file: &mut tokio::fs::File) -> Result<PlotHeader, Error> {
    ...
    validate_plot_header(&header, file.metadata().await?.len())?;
    Ok(header)
}
...
mod header_tests {
    ...
    fn uncompressed_header() -> PlotHeaderV1 {
        PlotHeaderV1 {
            ...
            k: 18,
            memo_len: 112,
            table_begin_pointers: std::array::from_fn(|index| 512 + index as u64 * 64),
            ..Default::default()
        }
    }

    fn compressed_header() -> PlotHeaderV2 {
        PlotHeaderV2 {
            magic: HEADER_V2_MAGIC,
            version: 2,
            k: 32,
            memo_len: 128,
            compression_level: 1,
            plot_flags: 1,
            table_begin_pointers: [0, 4608, 5120, 5632, 6144, 6656, 7168, 4096, 4160, 4224],
            table_sizes: [0, 512, 512, 512, 512, 512, 512, 64, 64, 384],
            ..Default::default()
        }
    }

    #[test]
    fn untrusted_lengths_and_truncated_fields_return_errors() {
        let mut bytes = [0u8; 320];
        bytes[52..54].copy_from_slice(&u16::MAX.to_be_bytes());
        assert!(parse_v1(&bytes).is_err());
        bytes[41..43].copy_from_slice(&u16::MAX.to_be_bytes());
        assert!(parse_v2(&bytes).is_err());
        for length in 0..54 {
            assert!(parse_v1(&bytes[..length]).is_err());
            assert!(parse_v2(&bytes[..length]).is_err());
        }
    }

    #[test]
    fn c3_delta_count_is_bounded_before_indexing() {
        let maximum = K_CHECKPOINT1INTERVAL as usize;
        assert!(validate_c3_delta_count(0, maximum).is_err());
        assert!(validate_c3_delta_count(maximum + 1, maximum + 1).is_err());
        assert!(validate_c3_delta_count(2, 1).is_err());
        assert!(validate_c3_delta_count(1, maximum).is_ok());
        assert!(validate_c3_delta_count(maximum, maximum).is_ok());
    }

    #[test]
    fn semantic_header_validation_preserves_physical_table_layouts() {
        assert!(validate_plot_header(&PlotHeader::V1(uncompressed_header()), 1152).is_ok());
        assert!(validate_plot_header(&PlotHeader::V2(compressed_header()), 7680).is_ok());
        let mut compressed = compressed_header();
        compressed.compression_level = 9;
        compressed.table_begin_pointers[1] = 0;
        compressed.table_sizes[1] = 0;
        assert!(validate_plot_header(&PlotHeader::V2(compressed), 7680).is_ok());
        let mut reordered = uncompressed_header();
        reordered.table_begin_pointers.rotate_right(3);
        assert!(validate_plot_header(&PlotHeader::V1(reordered), 1152).is_ok());
    }

    #[test]
    fn semantic_header_validation_rejects_unsafe_parameters_and_ranges() {
        for invalid_k in [0, 17, 51, 255] {
            let mut header = uncompressed_header();
            header.k = invalid_k;
            assert!(validate_plot_header(&PlotHeader::V1(header), 1152).is_err());
        }
        for invalid_address in [0, 1, 1152, u64::MAX] {
            let mut header = uncompressed_header();
            header.table_begin_pointers[8] = invalid_address;
            assert!(validate_plot_header(&PlotHeader::V1(header), 1152).is_err());
        }
        let mut header = uncompressed_header();
        header.table_begin_pointers[8] = header.table_begin_pointers[7];
        assert!(validate_plot_header(&PlotHeader::V1(header), 1152).is_err());
        let mut header = uncompressed_header();
        header.table_begin_pointers[9] = 1 << 30;
        assert!(validate_plot_header(&PlotHeader::V1(header), (1 << 30) + 64).is_err());
        let mut header = compressed_header();
        header.compression_level = 10;
        assert!(validate_plot_header(&PlotHeader::V2(header), 7680).is_err());
        let mut header = compressed_header();
        header.table_sizes[8] = u64::MAX;
        assert!(validate_plot_header(&PlotHeader::V2(header), 7680).is_err());
        let mut header = compressed_header();
        header.table_sizes[8] = 65;
        assert!(validate_plot_header(&PlotHeader::V2(header), 7680).is_err());
        let mut header = compressed_header();
        header.table_sizes[0] = 1;
        assert!(validate_plot_header(&PlotHeader::V2(header), 7680).is_err());
    }

    #[test]
    fn compressed_og_header_uses_all_336_bytes() {
        let header = compressed_header();
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&HEADER_V2_MAGIC);
        bytes.extend_from_slice(&header.version.to_le_bytes());
        bytes.extend_from_slice(&[0; 32]);
        bytes.push(header.k);
        bytes.extend_from_slice(&header.memo_len.to_be_bytes());
        bytes.extend_from_slice(&[0; 128]);
        bytes.extend_from_slice(&header.plot_flags.to_le_bytes());
        bytes.push(header.compression_level);
        for pointer in header.table_begin_pointers {
            bytes.extend_from_slice(&pointer.to_be_bytes());
        }
        for size in header.table_sizes {
            bytes.extend_from_slice(&size.to_le_bytes());
        }
        assert_eq!(bytes.len(), 336);
        assert!(parse_v2(&bytes[..320]).is_err());
        let parsed = parse_v2(&bytes).unwrap();
        assert_eq!(parsed.table_sizes, header.table_sizes);
        assert!(validate_plot_header(&PlotHeader::V2(parsed), 7680).is_ok());
    }
}
