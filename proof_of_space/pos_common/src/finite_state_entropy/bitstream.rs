...
impl<'a> BitDstream<'a> {
    ...
    pub fn new(src: &'a [u8], src_size: usize) -> Result<Self, Error> {
        if src_size < 1 || src_size > src.len() {
...
#[cfg(test)]
mod bounds_tests {
    use super::BitDstream;

    #[test]
    fn decoder_rejects_declared_sizes_outside_the_buffer() {
        assert!(BitDstream::new(&[], 1).is_err());
        assert!(BitDstream::new(&[1], 2).is_err());
        assert!(BitDstream::new(&[1], usize::MAX).is_err());
    }
}
