    ...
    if let Some(coin) = get_most_recent_singleton_coin_from_coin_spend(last_coin_spend)? {
        let parent_info = if coin.parent_coin_info == launcher_coin.name() {
            ...
            if let Some(last_coin_spend_inner_puzzle) = get_inner_puzzle_from_puzzle(&p)? {
                ...
                return Err(Error::new(ErrorKind::InvalidInput, "Invalid Inner Puzzle when calculating parent info"));
            }
        };
        let full_solution: SerializedProgram = SerializedProgram::from(
            Program::to(vec![parent_info.to_sexp(), last_coin_spend.coin.amount.to_sexp(), inner_sol.to_sexp()])
        );
        let full_puzzle: SerializedProgram = SerializedProgram::from(
        ...
        if coin.puzzle_hash != full_puzzle_program.tree_hash() {
            return Err(Error::new(ErrorKind::InvalidInput, "Coin puzzleHash and Puzzle Treehash are not Equal"));
        }
        let reward_parent = pool_parent_id(height, genesis_challenge);
        let p2_singleton_puzzle: SerializedProgram = SerializedProgram::from(
        ...
        let p2_singleton_solution: SerializedProgram = SerializedProgram::from(
        ...
        if get_inner_puzzle_from_puzzle(&full_puzzle_program)?.is_none() {
            return Err(Error::new(ErrorKind::InvalidInput, "Failed to get inner puzzle from full puzzle"));
        }
        let coin_spends = vec![
            CoinSpend {
                coin,
        ...
        Err(Error::new(ErrorKind::InvalidInput, "Failed to find most recent singleton coin from coin spend"))
    }
}
