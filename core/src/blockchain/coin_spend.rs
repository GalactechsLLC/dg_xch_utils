...
pub fn compute_additions_with_cost(cs: &CoinSpend, max_cost: u64) -> Result<(Vec<Coin>, u64), Error> {
    let parent_coin_info = cs.coin.name();
    let mut ret: Vec<Coin> = vec![];
    let (mut cost, r) = cs.puzzle_reveal.run_with_cost(max_cost, &cs.solution.to_program()?)?;
    for cond in Program::to(r).as_list(){
        if cost > max_cost {
            return Err(Error::new(ErrorKind::Other, "BLOCK_COST_EXCEEDS_MAX compute_additions() for CoinSpend"));
        } else {
            ...
            if atoms.len() < 3 {
                return Err(Error::new(ErrorKind::Other, "Invalid Number ot Atoms in Program"));
            }
