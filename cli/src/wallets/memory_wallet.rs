...
impl WalletStore for MemoryWalletStore {
    ...
    async fn get_max_send_amount(&self) -> u128 {
        if self.unspent_coins.is_empty() {
            ...
            self.unspent_coins.values().map(|v| v.coin.amount as u128).sum()
        }
    }
    ...
    async fn select_coins(&self, amount: u64, exclude: Option<&[Coin]>, min_coin_amount: Option<u64>, max_coin_amount: u64, exclude_coin_amounts: Option<&[u64]>) -> Result<HashSet<Coin>, Error> {
        ...
        if amount as u128 > spendable_amount {
            ...
            for (_, coin_record) in &self.unspent_coins {
                if exclude.contains(&coin_record.coin) {
                    continue
                }
                if coin_record.coin.amount < min_coin_amount || coin_record.coin.amount > max_coin_amount {
                    continue
                }
            ...
            if amount == 0 && sum_spendable_coins == 0 {
                return Err(Error::new(ErrorKind::InvalidInput, format!("No coins available to spend, you can not create a coin with an amount of 0, without already having coins.")));
            }
            valid_spendable_coins.sort_by(|f, s| {
                f.amount.cmp(&s.amount)
            });
            match check_for_exact_match(&valid_spendable_coins, amount) {
                ...
                None => {
                    ...
                    if smaller_coin_sum == amount && smaller_coins.len() < max_num_coins && amount != 0 {
                        ...
                        let smallest_coin = select_smallest_coin_over_target(amount, &valid_spendable_coins);
                        ...
                        let mut coin_set = knapsack_coin_algorithm(&smaller_coins, amount, max_coin_amount, max_num_coins, None);
                        debug!("Selected coins from knapsack algorithm: {:?}", coin_set);
                        if coin_set.is_none() {
                            coin_set = sum_largest_coins(amount as u128, &smaller_coins);
                            if coin_set.is_none() || coin_set.as_ref().map(|v| v.len()).unwrap_or_default() > max_num_coins {
                                let greater_coin = select_smallest_coin_over_target(amount, &valid_spendable_coins);
                        ...
                        coin_set.ok_or_else(|| {
                            Error::new(ErrorKind::InvalidInput, "Failed to select coins for transaction")
                        })
                    } else {
                        match select_smallest_coin_over_target(amount, &valid_spendable_coins) {
                            ...
                            None => {
                                Err(Error::new(ErrorKind::InvalidInput, "Too many coins are required to make this transaction"))
                            }
                        }
                    }
                }
    ...
    async fn populate_secret_key_for_puzzle_hash(&self, puz_hash: &Bytes32) -> Result<Bytes48, Error> {
        ...
        match self.keys_for_ph.get(puz_hash) {
            None => {
                ...
                Err(Error::new(ErrorKind::NotFound, format!("Failed to find puzzle hash: {puz_hash})")))
            }
            Some(v) => {
                let secret_key = SecretKey::from_bytes(v.value().0.as_ref())
                    .map_err(|e| Error::new(ErrorKind::InvalidInput, format!("MasterKey: {:?}", e)))?;
                let synthetic_secret_key = calculate_synthetic_secret_key(&secret_key, &DEFAULT_HIDDEN_PUZZLE_HASH)?;
    ...
    async fn populate_secret_keys_for_coin_spends(&self, coin_spends: &[CoinSpend]) -> Result<(), Error> {
        for coin_spend in coin_spends {
            self.populate_secret_key_for_puzzle_hash(&coin_spend.coin.puzzle_hash).await?;
        }
        Ok(())
    }
    ...
    async fn secret_key_for_public_key(&self, public_key: &Bytes48) -> Result<SecretKey, Error> {
        match self.secret_key_store.secret_key_for_public_key(public_key) {
            None => {
                Err(Error::new(ErrorKind::NotFound, format!("Failed to find public_key: {public_key})")))
            }
            Some(v) => {
                let secret_key = SecretKey::from_bytes(v.value().as_ref())
                    .map_err(|e| Error::new(ErrorKind::InvalidInput, format!("MasterKey: {:?}", e)))?;
                Ok(secret_key)
            }
        }
...
fn knapsack_coin_algorithm(
    ...
    let mut seed = Bytes32::new(seed.unwrap_or(b"knapsack seed"));
    ...
    for i in 0..1000 {
        ...
        while n_pass < 2 && !target_reached {
            for coin in smaller_coins{
                if (n_pass == 0 && rand.gen::<bool>()) || (n_pass == 1 && !selected_coins.contains(coin)) {
...
impl Wallet<MemoryWalletStore, MemoryWalletConfig> for MemoryWallet {
    fn create(info: WalletInfo<MemoryWalletStore>, config: MemoryWalletConfig) -> Self {
        let fullnode_client = FullnodeClient::new(&config.fullnode_host.clone(), config.fullnode_port, config.fullnode_ssl_path.clone(), &config.additional_headers.clone());
        Self {
            info,
            config,
            fullnode_client
