...
use dg_xch_puzzles::p2_delegated_puzzle_or_hidden_puzzle::{puzzle_for_pk, solution_for_conditions};
...
pub trait Wallet<T: WalletStore + Send + Sync, C> {
    ...
    fn make_solution(
        ...
        if !primaries.is_empty() {
            for primary in primaries {
                condition_list.push(make_create_coin_condition(primary.puzzlehash, primary.amount, &primary.memos));
            }
        }
        if min_time > 0 {
    ...
    fn compute_memos(&self, _spend_bundle: &SpendBundle) -> Result<Vec<(Bytes32, Vec<Vec<u8>>)>, Error> {
        todo!()
    }
    ...
    async fn generate_signed_transaction(
        ...
        let transaction = self.generate_unsigned_transaction(
            amount,
            puzzle_hash,
            fee,
            origin_id,
        ...
        let spend_bundle = sign_coin_spends(
            ...
            self.wallet_info().constants.max_block_cost_clvm.to_u64().unwrap(),
        )
            .await?;
    ...
    async fn generate_unsigned_transaction(
        ...
        if !ignore_max_send_amount {
            ...
            debug!("Got back max send amount: {}", max_send);
        }
        ...
        if coins.is_none() {
            ...
            return Err(Error::new(ErrorKind::InvalidInput, "Can't exclude coins when also specifically including coins"));
        } else {
        ...
        assert!(change >= 0);
        let coin_announcements_bytes = coin_announcements_to_consume.unwrap_or_default().iter().map(|a| a.name()).collect::<Vec<Bytes32>>();
        let puzzle_announcements_bytes = puzzle_announcements_to_consume.unwrap_or_default().iter().map(|a| a.name()).collect::<Vec<Bytes32>>();
        let mut spends: Vec<CoinSpend> = vec![];
        let mut primary_announcement_hash = None;
        if primaries.is_some() {
            let mut all_primaries_list = primaries.unwrap_or_default().iter().map(|a| Primary {
            ...
            if all_primaries_list.len() != as_set.len() {
                return Err(Error::new(ErrorKind::InvalidInput, format!("Cannot create two identical coins")));
            }
        }
            ...
            if [None, Some(coin.name())].contains(&origin_id) {
                ...
                if change > 0 {
                    if reuse_puzhash {
                        ...
                        for primary in &primaries {
                            if change_puzzle_hash == primary.puzzlehash && change == primary.amount as i128 {
                        ...
                        let change_puzzle_hash = self.get_new_puzzlehash().await?;
                        primaries.push(AmountWithPuzzlehash {
                ...
                let solution = self.make_solution(
                    &primaries,
                    0,
                    Some(HashSet::from([message.clone()])),
                    Some(HashSet::from_iter(coin_announcements_bytes)),
                    None,
                    Some(HashSet::from_iter(puzzle_announcements_bytes)),
                    fee,
                )?;
                primary_announcement_hash = Some(Announcement{
