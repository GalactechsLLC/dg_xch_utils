...
pub async fn test_full_node_client() -> Result<(), std::io::Error> {
    ...
    let ssl_path = env::var("FULLNODE_SSL_PATH")
        .ok()
        .unwrap_or(String::from("./ssl"));
    let client = FullnodeClient::new(
        &hostname,
        port,
        10,
        Some(ClientSSLConfig {
            ssl_crt_path: format!("{}/{}", ssl_path, "full_node/private_full_node.crt"),
            ssl_key_path: format!("{}/{}", ssl_path, "full_node/private_full_node.crt"),
            ssl_ca_crt_path: format!("{}/{}", ssl_path, "full_node/private_full_node.crt"),
        }),
        &None,
    );
