...
use crate::utils::clvm_puzzles::{get_most_recent_singleton_coin_from_coin_spend, solution_to_pool_state};
...
lazy_static! {
    pub static ref SINGLETON_LAUNCHER_PUZZLE: Program = SerializedProgram::from_hex(SINGLETON_LAUNCHER_HEX).unwrap().to_program().unwrap();
    pub static ref SINGLETON_LAUNCHER_PUZZLE_HASH: Bytes32 = SINGLETON_LAUNCHER_PUZZLE.tree_hash();
}

pub async fn scrounge_for_plotnfts(client: &FullnodeClient, puzzle_hashes: &[Bytes32]) -> Result<Vec<PlotNft>, Error> {
    let hashes = client.get_coin_records_by_puzzle_hashes(puzzle_hashes, Some(true), None, None).await?;
...
pub async fn get_plotnft_by_launcher_id(client: &FullnodeClient, launcher_id: &Bytes32) -> Result<Option<PlotNft>, Error> {
