...
use crate::types::blockchain::sized_bytes::{hex_to_bytes, Bytes32, Bytes48};
use crate::wallet::util::curry_and_treehash::{
...
pub fn calculate_synthetic_offset(public_key: &Bytes48, hidden_puzzle_hash: &Bytes32) -> BigInt {
    let blob = hash_256(
        public_key
            .to_sized_bytes()
            .into_iter()
            .chain(hidden_puzzle_hash.to_sized_bytes().into_iter())
            .collect::<Vec<u8>>(),
    );
