...
fn import_wallet_with_mnemonic(gui: &mut DgXchGui, mnemonic_vec: &Vec<String>) -> Result<(SecretKey, SecretKey), String> {
    ...
    match key_from_mnemonic_str(&mnemonic_str) {
        Ok(sk) => {
            master_sk = sk;
        }
        Err(_) => {}
    ...
    match master_sk_to_wallet_sk(&master_sk, 0) {
        Ok(sk) => {
            wallet_sk = sk;
        }
        Err(_) => {}
