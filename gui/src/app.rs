...
impl DgXchGui {
    pub fn new(_cc: &eframe::CreationContext<'_>, config: Config, shutdown_signal: Arc<AtomicBool>) -> Result<Self, std::io::Error> {
        let app = Self {
            state: State {
                ...
                full_node_client: Arc::new(FullnodeClient::new(
                    ...
                    10,
                    config.full_node_config.full_node_ssl.clone().map(|v| ClientSSLConfig {
...
impl eframe::App for DgXchGui {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        TAB_BAR.get_or_init(|| {
            Mutex::new(TabBar::new(self))
        }).lock().update(self, ctx, frame);
        egui::CentralPanel::default().show(ctx, |_ui| {
            match self.state.selected_tab {
                SelectedTab::Farmer => {
                    FARMER_SCENE.get_or_init(|| {
                        Mutex::new(FarmerScene::new())
                    }).lock().update(self, ctx, frame)
                },
                SelectedTab::Wallet => {
                ...
                SelectedTab::FullNode => {
                    FULL_NODE_SCENE.get_or_init(|| {
                        Mutex::new(FullNodeScene::new(self))
                    }).lock().update(self, ctx, frame)
                },
                SelectedTab::Config => {
                    CONFIG_SCENE.get_or_init(|| {
                        Mutex::new(ConfigScene::new())
                    }).lock().update(self, ctx, frame)
                },
            }
        });
