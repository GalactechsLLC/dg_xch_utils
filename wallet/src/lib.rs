...
pub struct SecretKeyStore {
    keys: DashMap<Bytes48, zeroize::Zeroizing<[u8; 32]>>,
}
impl SecretKeyStore {
    #[must_use]
    pub fn save_secret_key(&self, secret_key: &SecretKey) -> Option<zeroize::Zeroizing<[u8; 32]>> {
        self.keys.insert(
            Bytes48::from(secret_key.sk_to_pk()),
            zeroize::Zeroizing::new(secret_key.to_bytes()),
        )
    }
    #[must_use]
    pub fn secret_key_for_public_key(
        &self,
        pub_key: &Bytes48,
    ) -> Option<Ref<'_, Bytes48, zeroize::Zeroizing<[u8; 32]>>> {
...
pub trait WalletStore {
    ...
    async fn get_unused_derivation_record(
        ...
        if self.current_index() == u32::MAX {
            return Err(Error::other("wallet derivation index exhausted"));
        }
    ...
    async fn select_coins(
        ...
        if amount as u128 > spendable_amount {
            ...
            let mut sum_spendable_coins = 0u128;
                ...
                .filter(|v| !v.spent)
            {
                ...
                sum_spendable_coins += u128::from(coin_record.coin.amount);
                valid_spendable_coins.push(coin_record.coin);
            }
            if sum_spendable_coins < u128::from(amount) {
            ...
            if let Some(c) = check_for_exact_match(&valid_spendable_coins, amount) {
                ...
                for coin in &valid_spendable_coins {
                    if coin.amount < amount {
                        smaller_coin_sum += u128::from(coin.amount);
                        smaller_coins.push(*coin);
                    }
                    all_sum += u128::from(coin.amount);
                }
                if smaller_coin_sum == u128::from(amount)
                ...
                } else if smaller_coin_sum < u128::from(amount) {
                    let smallest_coin =
...
fn knapsack_coin_algorithm(
    ...
    let mut best_set_sum = u128::from(max_coin_amount);
        ...
        while n_pass < 2 && !target_reached {
            for coin in smaller_coins {
                    ...
                    || (n_pass == 1 && !selected_coins.contains(coin))
                {
                    ...
                    selected_coins_sum += u128::from(coin.amount);
                    selected_coins.insert(*coin);
                    match selected_coins_sum.cmp(&u128::from(target)) {
...
pub trait Wallet<T: WalletStore + Send + Sync, C> {
    ...
    async fn puzzle_hashes(
        ...
        if end > u32::MAX as usize {
            return Err(Error::other("wallet derivation index exceeds u32"));
        }
    ...
    async fn generate_signed_transaction(
        ...
        let non_change_amount = if let Some(primaries) = primaries {
            primaries
                .iter()
                .try_fold(amount, |total, payment| total.checked_add(payment.amount))
                .ok_or_else(|| Error::other("transaction amount overflow"))?
        } else {
            amount
        };
        debug!(
        ...
        if transaction.is_empty() {
            return Err(Error::other("transaction has no coin spends"));
        }
        ...
        let output_amount: u128 = add_list
            .iter()
            .map(|coin| u128::from(coin.amount))
            .sum::<u128>()
            + u128::from(fee);
        let input_amount: u128 = rem_list.iter().map(|coin| u128::from(coin.amount)).sum();
        if (negative_change_allowed && output_amount < input_amount)
            || (!negative_change_allowed && output_amount != input_amount)
        {
            return Err(Error::other(
                "signed transaction does not conserve the requested amount and fee",
            ));
        }
    ...
    async fn generate_unsigned_transaction(
        ...
        let mut primaries_amount = 0u128;
        let total_amount: u128;
        if let Some(primaries) = primaries {
            for primary in primaries {
                primaries_amount += u128::from(primary.amount);
            }
            total_amount = amount as u128 + fee as u128 + primaries_amount;
        } else {
            total_amount = amount as u128 + fee as u128;
        }
        let reuse_puzhash = reuse_puzhash.unwrap_or(true);
        if total_amount == 0 || total_amount > u128::from(u64::MAX) {
            return Err(Error::new(
                ErrorKind::InvalidInput,
        ...
        if coins_set.is_empty() {
            return Err(Error::new(ErrorKind::InvalidInput, "no spendable coins"));
        }
        ...
        if change < 0 || change > i128::from(u64::MAX) {
            return Err(Error::new(
                ErrorKind::InvalidInput,
...
pub fn compute_memos_for_spend(
    ...
    for condition in result_list {
        ...
        if conditions.len() >= 4
            && ConditionOpcode::from(conditions[0].sexp()) == ConditionOpcode::CreateCoin
        {
