use crate::common::sign_coin_spend;
use crate::memory_wallet::{MemoryWalletConfig, MemoryWalletStore};
use crate::{Wallet, WalletInfo, WalletStore};
...
impl Wallet<MemoryWalletStore, MemoryWalletConfig> for PlotNFTWallet {
    ...
    async fn create_spend_bundle(
        ...
        Err(Error::new(
            ...
            "generic PlotNFT spending is not implemented; use the validated travel transaction path",
        ))
    }
}
impl PlotNFTWallet {
    ...
    pub async fn generate_travel_transaction(
        ...
        let (additions, _cost) = last_coin_spend.compute_additions_with_cost(
            constants
                .max_block_cost_clvm
                .to_u64()
                .ok_or_else(|| Error::other("CLVM cost limit exceeds u64"))?,
        )?;
        let singleton = additions
            .first()
            .ok_or_else(|| Error::other("singleton spend has no additions"))?;
        let singleton_id = singleton.name();
        if outgoing_coin_spend.coin.parent_coin_info != last_coin_spend.coin.name()
            || outgoing_coin_spend.coin.name() != singleton_id
            || new_inner_puzzle == inner_puzzle
        {
            return Err(Error::other("invalid singleton travel transition"));
        }
        ...
        .await?;
        if signed_spend_bundle.removals().first() != Some(singleton) {
            return Err(Error::other(
                "signed travel transaction spends the wrong singleton",
            ));
        }
...
pub async fn scrounge_for_plotnfts(
    ...
    Ok(Arc::try_unwrap(plotnfts)
        .map_err(|_| Error::other("PlotNFT scan tasks still own the result"))?
        .into_inner())
}
...
pub async fn submit_next_state_spend_bundle(
    ...
    let coin_to_find = travel_record
        ...
        .iter()
        .find(|c| c.amount == 1)
        .ok_or_else(|| Error::other("failed to find NFT coin"))?;
    match client
        .push_tx(
            &travel_record
                .spend_bundle
                .ok_or_else(|| Error::other("transaction record has no spend bundle"))?,
        )
        .await?
    {
        TXStatus::SUCCESS => {
